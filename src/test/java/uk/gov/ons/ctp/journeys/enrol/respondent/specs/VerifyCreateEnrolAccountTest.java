package uk.gov.ons.ctp.journeys.enrol.respondent.specs;

import org.junit.runner.RunWith;

import cucumber.api.CucumberOptions;
import cucumber.api.junit.Cucumber;

/**
 * Created by stephen.goddard on 28/06/17.
 */
@RunWith(Cucumber.class)
@CucumberOptions(features = {"classpath:uk/gov/ons/ctp/journeys/enrol/respondent/verifyCreateEnrolAccount.feature"},
  glue = {"uk.gov.ons.ctp.response.samplesvc.steps",
      "uk.gov.ons.ctp.response.collectionexercisesvc.steps",
      "uk.gov.ons.ctp.response.iacsvc.steps",
      "uk.gov.ons.ctp.response.casesvc.steps",
      "uk.gov.ons.ctp.response.common.steps"},
  plugin = {"pretty", "html:build/enrol-cuc-html-report", "json:build/jenkins/enrol.json"}
)
public class VerifyCreateEnrolAccountTest {

}
