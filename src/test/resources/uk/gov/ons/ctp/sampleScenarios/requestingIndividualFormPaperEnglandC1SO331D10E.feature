# Author: Edward Stevens 21/12/2016
...
#
# Feature Tags: @sampleScenarios @requestingIndividualFormPaperEnglandC1SO331D10E
#
...
@sampleScenarios @requestingIndividualFormPaperEnglandC1SO331D10E
Feature: Request individual form letter - England C1S0331D10E
  ...
  Scenario: Check case has changed to ACTIONABLE and removed from action.case
    ...
    And check "action.case" records in DB equal 1 for "caseid = 5"
  ...
  Scenario: Test action plan has created paper file for printer
    ...
    Then get the contents of the file from "/var/actionexporter-sftp/" where the filename begins "I1S"
    And the exit status should be 0
    When and each line should start with an iac
    And and the contents should contain "|1000000000000011||dr|Integration|Tester||5 LOWE COURT|WELLINGTON|TELFORD|TF1 1LZ"
