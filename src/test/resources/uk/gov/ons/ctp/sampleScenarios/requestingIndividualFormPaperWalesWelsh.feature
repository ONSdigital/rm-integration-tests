...
# Keywords Summary : This feature file contains the scenario testing the requesting an individual paper form to Wales in Welsh.
...
#
# Feature Tags: @sampleScenarios @requestingIndividualFormPaperWalesWelsh
#
...
@sampleScenarios @requestingIndividualFormPaperWalesWelsh
Feature: Request individual form letter - Wales (Welsh)
  ...
  Scenario: Put request for sample to create online cases for the specified sample id
    ...
    And the response should contain the field "sampleCaseTypes" with one element of the JSON array must be {"caseTypeId":28,"respondentType":"HI"}
  ...
  Scenario: Request a paper form
    ...
    And selects case for address "58 CALDERA ROAD"
    And navigates to the cases page for case "9"
    And the user requests an individual request for
      | Paper | Dr | Integration | Tester |  | 07777123456 | Welsh |
  ...
  Scenario: Test action plan has created paper file for printer
    ...
    Then get the contents of the file from "/var/actionexporter-sftp/" where the filename begins "I2"
    And the exit status should be 0
    When and each line should start with an iac
    And and the contents should contain "|1000000000000011||dr|Integration|Tester||58 CALDERA ROAD|HADLEY|TELFORD|TF1 5LT"
