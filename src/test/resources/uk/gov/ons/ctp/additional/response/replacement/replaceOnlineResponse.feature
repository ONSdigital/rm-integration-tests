...
# Keywords Summary : This feature file contains the scenario tests for replacement online response HH and individual.
...
#								 CTPA-673 Generate Internet Access Code for each case created.
#								 CTPA-579 Generate message for Notify
#								 CTPA-727 Send to notify
#								 CTPA-651 Validate IAC
#								 CTPA-774 Update Case
...
#								 @replaceOnlineResponseReceived
#								 @replaceDistributedActionsCancelled
#								 @replaceValidateIAC
#								 @replaceOnlineRespondedUI

@replaceOnlineResponse @additionalReplacementResponse
Feature: Test successfull replacements for HH and individual online response
  ...
  Scenario: Confirm old individual case IAC has been deactivated
	...
	Scenario: Get request to case newly created case
		...
		And the response should contain the field "state" with value "ACTIONABLE"
		And the response should contain the field "caseTypeId" with an integer value of 17
		And the response should contain the field "actionPlanMappingId" with an integer value of 39
		And the response should contain the field "createdDateTime"
		And the response should contain the field "createdBy" with value "integration.tester"
		And the response should contain the field "iac"
		And the response should contain the field "responses" with one element of the JSON array must be []
		And the response should contain the field "contact" with list value "{title=rev, forename=Integration, surname=Tester, phoneNumber=07777123456, emailAddress=null}"
	...
	Scenario: Get request to case newly created case
		...
		And the response should contain the field "iac"
		And the response should contain the field "responses" with one element of the JSON array must be []
		And the response should contain the field "contact" with list value "{title=prof, forename=Integration, surname=Tester, phoneNumber=07777123456, emailAddress=null}"
  ...
  Scenario: Test the system has successfully registered an online response
  	Given I make the POST call to the SDX Gateway online receipt for caseref "1000000000000012"
	...
	@replaceOnlineResponseReceived
  Scenario: Test the system has successfully registered an online response
  	Given I make the POST call to the SDX Gateway online receipt for caseref "1000000000000013"
		And the response status should be 201
		When after a delay of 30 seconds
